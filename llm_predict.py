...
from utils.data_utils import get_user_profile, retrieve_api_embedding, UserRetriever, standardize_keys, process_triplet, load_or_create_dataframe, update_dataframe, remove_dup
...
parser.add_argument('--nb_hist_only', default=0, type=int)
parser.add_argument('--include_rf', default=1, type=int) #1 rf 2 r 3 both
...
# new_user_data = load_file(f"{CNN_dir}users_info_dict_with_chatgptanno.json")
# new_user_data = load_file(f"{CNN_dir}users_info_dict_with_chatgptanno_rf.json")
# new_user_data_sc = load_file(f"{CNN_dir}users_info_dict_with_chatgptanno_sc.json")
# new_user_data_sc = load_file(f"{CNN_dir}users_info_dict_with_chatgptanno_r.json")
new_user_data_sc = load_file(CNN_users_info_rf_filename) #

# Prompt Types
# prompt_types_tmp = ['prediction_with_prompt_sc']  # "prediction_with_prompt_sc",
# prompt_types_tmp = ['prediction', 'prediction_with_prompt', 'prediction_with_prompt_sc']  # "prediction_with_prompt_sc",
# prompt_types_tmp = ['prediction_with_prompt_s', 'prediction_with_prompt_r', 'prediction_with_prompt_hists', 'prediction_with_prompt_s']  #
# prompt_types_tmp = ['prediction', 'prediction_with_prompt_s', 'prediction_with_prompt_r', 'prediction_with_prompt_hists']  # "prediction_with_prompt_sc",
# prompt_types_tmp = ['prediction_with_prompt_s']  # "prediction_with_prompt_sc",
# prompt_types_tmp = ['prediction_with_prompt_hists', 'prediction_with_prompt_histsr']  # "prediction_with_prompt_sc",
# prompt_types_tmp = ['prediction_with_prompt_hists_ret', 'prediction_ret']  # "prediction_with_prompt_sc",
# prompt_types_tmp = ['prediction_ret']  # "prediction_with_prompt_sc",
# prompt_types_tmp = ['prediction_with_prompt_hists_ret_collab', 'prediction_ret', 'prediction_with_prompt_hists_ret', 'prediction']
# prompt_types_tmp = ['prediction']  # "prediction_with_prompt_1sc",
# prompt_types_tmp = ['prediction_with_prompt_hists_ret_collab']  # "prediction_with_prompt_1sc",
prompt_types_tmp = args.prompt_types

# prompt_types_tmp = ['prediction', 'prediction_with_prompt_s']  # "prediction_with_prompt_sc",
prompt_types = []
...
if path_exists(f"{CNN_dir}case_study_dict.json"):
    ...
    # for k in ["longest_hist_100", "mixed_100", "lurker_50hist_nonempty"]:
    #     case_study_dict[f"{k}_test"] = case_study_dict[k] & valid_uids_test

# case_study_keys = ['', 'empty_profile', '100_hist', '50_hist', '20_hist', '10_hist', '0_hist', 'unseen_test']  # ,'0_hist_all','10_hist_all','50_hist_all'
# case_study_keys = ['20_hist']  # ,'0_hist_all','10_hist_all','50_hist_all'
# case_study_keys = ['100_hist', '50_hist', '20_hist', '10_hist', '0_hist', 'unseen_test']  # ,'0_hist_all','10_hist_all','50_hist_all'
# case_study_keys = ['']  # ,'0_hist_all','10_hist_all','50_hist_all'
# case_study_keys = ['longest_hist_100_all']  # ,'0_hist_all','10_hist_all','50_hist_all' 'longest_hist_100',
# case_study_dict['longest_hist_100_all'] = {v.split(".")[0] for v in os.listdir(f"{CNN_dir}chatgptanno_rf_json/")} # for traing with eval 100 users case, _Copy suffix
# case_study_keys = ['longest_hist_100']  # ,'0_hist_all','10_hist_all','50_hist_all' 'longest_hist_100',
# case_study_keys = ['']  # ,'0_hist_all','10_hist_all','50_hist_all'
...
# if not is_eval:
#     case_study_keys = ['']
# change filename with version number at end
if args.debug:
    case_study_keys = ['']
...
# if case_study_keys == ['longest_hist_100']:
...
eval_keys = ["spearman", "pearson", "mif1", 'maf1', 'mse','mae','wasserstein','Bleu_1', 'Bleu_2', "Bleu_3", "Bleu_4", 'meteor', 'rouge1', 'rouge2', 'rougeL', "avg_prompt_tokens", "avg_hist_tokens","pi_prompt", "pi_hist", "pi_prompt2", "pi_hist2"]

# eval_keys = ["spearman", "pearson", "mif1", 'maf1']

def make_llm_prediction(history, post_text, profile, prompt_type, use_refined_profile=1, augment=None, collab_history=None, args=args):
    ...
    cur_prompt_prediction = get_prompt(history=history, post=post_text, profile=profile,
                                       key=cur_key, nbs=None, sc=cur_sc, prompt_type=prompt_type, refine=cur_refine, neighbors=collab_history)
...
for j, prompt_type in enumerate(prompt_types):
    ...
    for case in case_study_keys:
        ...
        for i, sample in enumerate(tqdm(cur_test_anno)):  # this indicates ith path after breaking out all articles into individual paths
            ...
            tweet_id = sample["tweet_id"] if "tweet_id" in sample else f"{user_id}_{post_id}"
            # if path_exists(f"{CNN_dir}chatgpt_pred_{prompt_type}/{tweet_id}.txt"):
            #     # new_user_data[k]["chatgptanno"] = load_file(f"{CNN_dir}chatgptanno/{k}.txt")
            #     continue
            cur_top_n_hist = 100 if "_ret" in prompt_type else args.n_hist
            user_desc, history_text, history_posts, history_tweet_ids = get_user_profile(user_id, user_data, history_data, top_n_hist=cur_top_n_hist)
            ...
            predicted_intensity = int(sample['predicted_intensity'])
            ...
            if is_eval and path_exists(cur_pred_fname):  # and not args.eval_pi
                ...
                try:
                    decoded_pred, sent_pred, intensity_pred = parse_gpt_response(res, pure_int=bool(pure_intensity))
                except:
            ...
            len_tokens_limit = 2500
            if "hist" in prompt_type:  # for struc
                len_tokens_limit = 2000
            ...
            if "_lc" in prompt_type:
                ...
                history = llm_lingua.compress_prompt(history, rate=0.40, force_tokens=['\n', '?'])['compressed_prompt']
            ...
            if "_ret" in prompt_type:
                ...
                # get history refine
                if "rf" in args.modules: # struc needs to be in
                ...
                query = "Encode this news headline to find the most relevant user information that might hint at their reaction to it: " + news
                # print("News", news)

                # retrieve
                top_indices = retrieve_api_embedding((post_id, query), hvr, top_k=args.item_topk, run_name=run_name, save=True, threshold=0) #args.item_threshold
                ...
                if "collab" in prompt_type and ur.get_user_profile(user_id).strip():
                    # print("e0")
                    ...
                    # print("e1")
                    refined_history_vec = ur.join_db(retrieved_uids)
                    if args.do_process: refined_history_vec=remove_dup(refined_history_vec)
                    ...
                    hvr = [(hist_id, refined_history_prompt + hist_item) for hist_id, hist_item in refined_history_vec]
                    combined_history_vec = refined_history_vec
                    top_indices = retrieve_api_embedding((post_id, query), hvr, top_k=args.item_topk_collab, run_name=run_name, save=True, threshold=args.item_threshold)
                    # print("e3")
                    collab_history = "\n".join([combined_history_vec[i][1] for i in top_indices])
                    # print("e4")
            ...
            # print("user_id", user_id)
            pred_fname, cur_prompt_prediction = make_llm_prediction(history, post_text, profile, prompt_type, use_refined_profile=args.use_refined_profile, augment=augment, collab_history=collab_history, args=args)
